...
為了確保腳本編寫的便利性（使用中文名）與系統處理的一致性（使用英文識別碼），系統採用「名稱對應表」機制。

### 1. 對應原則
*   **腳本顯示名稱**: 用於 `SAY` 指令的說話者，或是 `SPRITE` 指令的角色識別碼。
*   **內部識別名稱**: 用於匹配資源路徑、高亮判斷邏輯。系統會自動進行轉小寫與修剪前後空白處理。

### 2. 設定格式範例
目前系統內建的 `characterNameMap` 配置範例：
...
### 3. 建議操作
*   **指令一致性**: 在 `SPRITE` 指令中定義角色 ID 時，應與 `SAY` 指令使用的名稱在對應表內保持一致。
    *   *正確範例*:
        ```text
        SPRITE|伊莉莎白|left|elizabeth_neutral
        SAY|伊莉莎白|你好。
        ```
    *   *此時系統會自動匹配「伊莉莎白」為 「elizabeth」，並正確高亮左側立繪。*
*   **旁白處理**: 若不填寫說話者（如 `SAY||...`），系統將自動將所有立繪亮度恢復為 1.0（正常亮度）。
